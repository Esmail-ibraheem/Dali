...
class DDPMSampler:
    ...
    def set_inference_timesteps(self, number_infernece_step = 50):
        self.number_iference_steps = number_infernece_step
        ratio = self.number_training_timesteps // self.number_iference_steps
        timestep = (np.arange(0, number_infernece_step) * ratio).round()[::-1].copy().astype(np.int64)
        self.timesteps = torch.from_numpy(timestep)
    
    def _get_previous_timestep(self, timestep: int) -> int:
        previous_step = timestep - self.number_training_timesteps // self.number_iference_steps
    ...
    def set_strength(self, strength = 1):
        ...
        """
        startstep = self.number_iference_steps - int(self.number_iference_steps * strength)
    ...
    def step(self, timestep: int, latents: torch.Tensor, model_output: torch.Tensor):
        ...
        alphas_product_timesteps = self.alphas_cumlative_product[t]
        alphas_product_timesteps_previous = self.alphas_cumlative_product[previous_step] if previous_step >=0 else self.one
        current_alphas_product_timestep = alphas_product_timesteps / alphas_product_timesteps_previous
        beta_timestep = 1 - alphas_product_timesteps
        beta_timestep_previous = 1 - alphas_product_timesteps_previous
        current_beta_timestep = 1 - current_alphas_product_timestep
        predict_original_sample = (latents - beta_timestep ** (0.5) * model_output) / alphas_product_timesteps
        predict_original_sample_coeff = (alphas_product_timesteps_previous ** (0.5) * current_beta_timestep) / beta_timestep
        predict_current_sample_coeff = current_alphas_product_timestep ** (0.5) * beta_timestep_previous / beta_timestep
        predict_previous_sample = predict_original_sample_coeff * predict_original_sample + predict_current_sample_coeff * latents
        variance = 0
        if t > 0:
    ...
    def add_noise(self, original_samples: torch.FloatTensor, timesteps: torch.IntTensor) -> torch.FloatTensor:
        ...
        alphas_product_timestep_squaroot = alphas_cumlative_product[timesteps] ** 0.5
        ...
        alphas_product_timestep_squaroot_mins = (1 - alphas_cumlative_product[timesteps]) ** 0.5 
...
class DecisionTransformer:
    pass 
class RinforcementLearningFromHumanFeedback:
    pass 
class GELU:
    pass
class FeedForward:
